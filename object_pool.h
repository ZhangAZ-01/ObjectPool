...
template <typename T>
auto object_pool<T>::get() -> smarter_pointer {
  if (!pool_.empty()) {
...
template <typename T>
auto object_pool<T>::get_shared() -> shared_pointer {
  if (!pool_.empty()) {
    pointer ptr = pool_.back();
    pool_.pop_back();
...
template <typename T>
void object_pool<T>::release(pointer ptr) {
...
template <typename T>
void object_pool<T>::release_all() {
